  ...
  ReentrancyGuard
{
  ...
  constructor(address marketplaceAddress, address scoreTokenAddress)
    ERC721("CC6", "CC6")
  {
  ...
  function createToken(string memory uri) public returns (uint256) {
    ...
    require(
      keccak256(bytes(firststr)) == keccak256("https://bafy"),
      "firststr not same"
    );
  ...
  function mintAsDonorFromAuthor(string memory uri, address _author)
    public
    payable
    nonReentrant
    returns (uint256)
  ...
  function tokensOfOwner(
    ...
    uint256 _range = _offset > _tokenCount
      ? 0
      : Math.min(_tokenCount - _offset, _rows);
