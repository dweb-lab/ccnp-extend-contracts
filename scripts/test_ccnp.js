...
const fs = require("fs")
const CONTRACT_ADDRESS_Simple =
  fs.readFileSync(".spt_addr").toString().trim() || "0x111"
const CONTRACT_ADDRESS_NFT =
  fs.readFileSync(".nft_addr").toString().trim() || "0x111"
...
async function main() {
  ...
  const wei_amount = utils.parseEther("40.0") // uint256 amount = 10 ether?
  const ret_transfer = await contract_simple.transfer(
    CONTRACT_ADDRESS_NFT,
    wei_amount,
  )
  const nft_addr_balance2 = await contract_simple.balanceOf(
    CONTRACT_ADDRESS_NFT,
  )
  ...
  const nft_addr_balance3 = await contract_simple.balanceOf(
    CONTRACT_ADDRESS_NFT,
  )
  ...
  console.log(
    "tokenOfOwnerByIndex: ",
    await contract.tokenOfOwnerByIndex(owner, 0),
  )

  const donation_amount = utils.parseEther("0.1")
  const ret_mintAsDonorFromAuthor = await contract.mintAsDonorFromAuthor(
    "https://bafy.yyy",
    owner,
    {
  ...
  const nft_addr_balance4 = await contract_simple.balanceOf(
    CONTRACT_ADDRESS_NFT,
  )
